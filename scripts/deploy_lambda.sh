...
aws lambda update-function-configuration \
  --function-name ${FUNCTION_NAME} \
  --region ${REGION} \
  --environment "Variables={DATABASE_URL=${DATABASE_URL},DATABASE_URL_DIRECT=${DATABASE_URL_DIRECT},DB_PROVIDER=${DB_PROVIDER:-supabase},OPENAI_API_KEY=${OPENAI_API_KEY},ANTHROPIC_API_KEY=${ANTHROPIC_API_KEY},NORMALIZER_PROVIDER=${NORMALIZER_PROVIDER:-openai},NORMALIZER_MODEL=${NORMALIZER_MODEL:-gpt-4o-mini},NORMALIZER_MAX_CONCURRENCY=${NORMALIZER_MAX_CONCURRENCY:-5},AWS_S3_BUCKET=${AWS_S3_BUCKET}}" \
  --output text \
  --query 'FunctionName'
